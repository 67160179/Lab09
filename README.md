# Todo

## ส่วนที่ 1: การทำความเข้าใจโค้ด

### คำถาม 1.1

เหตุใดจึงต้องแยก `duration` และ `timeLeft` ออกจากกัน อธิบายความแตกต่าง

**คำตอบ:**  
`duration` คือค่าระยะเวลาทั้งหมดของงานตั้งแต่เริ่มต้น ส่วน `timeLeft` คือเวลาที่เหลือในระหว่างการทำงานซึ่งลดลงเรื่อย ๆ
การแยกกันช่วยให้สามารถคืนค่าเวลาเดิมหรือคำนวณเปอร์เซ็นต์ความคืบหน้าได้ หากใช้ตัวเดียวจะไม่รู้ว่าเริ่มจากกี่วินาที

---

### คำถาม 1.2

`formatTime()` ฟังก์ชันนี้ทำความสำคัญอะไร? ถ้าไม่มีฟังก์ชันนี้ app จะเป็นยังไง

**คำตอบ:**  
ฟังก์ชันนี้ใช้จัดรูปแบบตัวเลขวินาทีให้เป็นรูปแบบนาที:วินาที เพื่อให้ผู้ใช้เข้าใจง่าย
ถ้าไม่ใช้ เวลาจะถูกแสดงเป็นตัวเลขยาว ๆ เช่น 1764 ซึ่งอ่านยากและไม่เหมาะกับแอปจับเวลา

---

### คำถาม 1.3

อธิบายข้อแตกต่างระหว่าง `setInterval()` และ `setTimeout()`

**คำตอบ:**  
`setInterval()` ทำงานครั้งเดียวตามเวลาที่กำหนด
`setTimeout()` ทำงานซ้ำเป็นช่วง ๆ
แอปใช้ interval เพื่อนับถอยหลัง ส่วน timeout ใช้กับการซ่อน notification

---

## ส่วนที่ 2: ฟังก์ชันการทำงาน

### คำถาม 2.1

เมื่อผู้ใช้คลิกปุ่ม “▶️ เริ่ม” ครั้งแรก เกิดอะไรขึ้นในลำดับใด

**คำตอบ:**

1. เเรียก toggleTimer

2. เช็คว่ายังไม่กำลังทำงาน

3. ตั้งสถานะกำลังทำงาน

4. สร้างตัวจับเวลาทุก 1 วินาที

5. ลดค่าเวลาคงเหลือ

6. อัปเดตหน้าจอ

7. เมื่อเหลือ 0 → แจ้งเตือน + เล่นเสียง + หยุด

---

### คำถาม 2.2

ทำไมต้อง clearInterval() ก่อนตั้ง `task.isRunning = false`

**คำตอบ:**  
เพื่อหยุดการนับก่อนเปลี่ยนสถานะ ไม่เช่นนั้นตัวจับเวลาจะยังรันอยู่เบื้องหลังและค่าจะยังลดต่อ

---

### คำถาม 2.3

ถ้า timer หมดเวลาแต่ไม่มี clearInterval() จะเกิดอะไรขึ้น

**คำตอบ:**  
ตัวเลขเวลาจะลดต่ำกว่า 0 และระบบจะเรียกแจ้งเตือนซ้ำเรื่อย ๆ รวมถึงกินทรัพยากรเครื่องโดยไม่จำเป็น

---

## ส่วนที่ 3: DOM และ UI

### คำถาม 3.1

renderTasks() ควรแยกเป็นฟังก์ชันใด

**คำตอบ:**

- showEmpty()

- createTaskElement()

- updateTimerColor()

- createButtons()

---

### คำถาม 3.2

อัปเดต UI อย่างไร และมีวิธีอื่นหรือไม่

**คำตอบ:**  
ตอนนี้คือวาด HTML ใหม่ทั้งหมดทุกครั้งที่เวลาเปลี่ยน
อีกวิธีคือแก้เฉพาะ element ที่เปลี่ยน หรือใช้ระบบ Virtual DOM

---

### คำถาม 3.3

keypress event สำคัญอย่างไร

**คำตอบ:**  
ช่วยให้เพิ่มงานด้วยปุ่ม Enter ได้ทันที ถ้าเอาออกจะต้องใช้เมาส์อย่างเดียว ทำให้ใช้งานช้าลง

---

## ส่วนที่ 4: Web Audio API

### คำถาม 4.1

หน้าที่ของการตั้งค่าเสียง

**คำตอบ:**

- 800 Hz → โทนเสียงเตือน
- sine → เสียงเรียบไม่แตก
- gain 0.3 → ความดังระดับกลาง

---

### คำถาม 4.2

ทำไมเสียง 0.5 วินาที

**คำตอบ:**  
เสียงสั้นพอให้รู้แต่ไม่รบกวน ถ้ายาวเกินไปจะรำคาญโดยเฉพาะเมื่อมีหลายงานเสร็จติดกัน

---

## ส่วนที่ 5: Array Methods

### คำถาม 5.1

filter() ใช้ทำอะไร และมีวิธีอื่นไหม

**คำตอบ:**  
สร้างรายการใหม่โดยตัด task ที่ต้องการลบทิ้ง
ทางเลือกคือหา index แล้ว splice

---

### คำถาม 5.2

map() ใช้ทำอะไร

**คำตอบ:**  
เปลี่ยนข้อมูล task เป็น HTML string
forEach ไม่เหมาะเพราะไม่คืนค่ากลับมา

---

## ส่วนที่ 6: ข้อบกพร่องและการแก้ไข

### คำถาม 6.1

เริ่ม 2 task พร้อมกัน

**คำตอบ:**  
แต่ละงานจะมีตัวจับเวลาของตัวเอง จึงทำงานได้ แต่จำนวน interval เพิ่มขึ้น

---

### คำถาม 6.2

รีเฟรชแล้ว task หาย

**คำตอบ:**  
เพราะข้อมูลเก็บในหน่วยความจำชั่วคราว วิธีแก้คือบันทึกลง LocalStorage

---

### คำถาม 6.3

ทดสอบเร็วขึ้น

**คำตอบ:**  
ลดเวลาทดสอบเหลือไม่กี่วินาที หรือเพิ่มอัตราการลดเวลาต่อรอบ

---

## ส่วนที่ 7: Enhancement

### คำถาม 7.1

รีเซ็ต timer

**คำตอบ:**

```js
// รีเซ็ตเวลา task กลับไปค่าเริ่มต้น (30 นาที)
function resetTimer(id) {
  // หา task จาก id
  const task = tasks.find((t) => t.id === id);
  if (!task) return; // ถ้าไม่เจอให้หยุด

  // หยุด timer ที่กำลังทำงานอยู่
  clearInterval(task.intervalId);

  // ตั้งเวลาใหม่เป็นค่าเริ่มต้น (duration)
  task.timeLeft = task.duration;

  // เปลี่ยนสถานะเป็นหยุด
  task.isRunning = false;

  // อัปเดตหน้าจอใหม่
  renderTasks();
}
```

### 7.2 วิธีใดในการเก็บ tasks ไว้ใน LocalStorage เพื่อไม่ให้หายเมื่อรีเฟรชหน้า

```js
// บันทึกข้อมูล tasks ลงใน LocalStorage ของ browser
// JSON.stringify แปลง array/object → string เพราะ LocalStorage เก็บได้แค่ string
localStorage.setItem("tasks", JSON.stringify(tasks));

// โหลดข้อมูล tasks จาก LocalStorage กลับมาใช้
// JSON.parse แปลง string > array/object กลับมาเป็นข้อมูลเดิม
// ถ้าไม่มีข้อมูล (ครั้งแรกที่เปิดเว็บ) ให้ใช้ array ว่างแทน
tasks = JSON.parse(localStorage.getItem("tasks")) || [];
```

### 7.3 ถ้าต้องการให้ผู้ใช้กำหนดเวลา timer เอง จะต้องแก้ไขฟังก์ชันไหนบ้าง

แก้:

- เพิ่มช่องกรอกเวลา
- addTask()
- การแสดงผลเวลา

### 8.1 ทำไม renderTasks() จึงต้องเรียกหลายครั้ง มีวิธี DRY ที่ดีกว่าหรือไม่

เพราะ UI ไม่เชื่อมกับ state โดยตรง จึงต้องวาดใหม่ทุกการเปลี่ยนค่า วิธีที่ดีกว่าคืออัปเดตเฉพาะจุด

### 8.2 ถ้าเปลี่ยนจาก setInterval เป็น requestAnimationFrame จะส่งผลดีหรือไม่

ไม่เหมาะ เพราะใช้กับ animation 60fps แต่ timer ต้องการความแม่นระดับวินาที

### 8.3 ในโค้ดที่ใช้ setInterval() มี Memory Leak ที่อาจเกิดขึ้นได้หรือไม่

อาจเกิดหากลบ task โดยไม่หยุด interval ก่อน

---

# Pomodoro

# Lab 09 — Pomodoro Timer

---

## ส่วนที่ 1: ความเข้าใจเทคนิค Pomodoro

### 1.1 เทคนิค Pomodoro คืออะไร เหตุใดจึงเลือกใช้เวลา 25 นาที

เป็นเทคนิคแบ่งเวลาทำงานเป็นช่วงสั้น ๆ เพื่อให้สมาธิไม่ล้า
25 นาทีถูกเลือกเพราะเป็นช่วงที่สมองโฟกัสได้เต็มที่ก่อนเริ่มเหนื่อย ทำให้ประสิทธิภาพสูงกว่าการทำยาว ๆ

---

### 1.2 หากต้องการปรับเวลา Work/Break ให้แตกต่างจาก default (25/5) จะต้องแก้ไขโค้ดส่วนไหน

- ค่าเริ่มต้นใน state (workDuration, breakDuration)

- input settings (workTimeInput, breakTimeInput)

- ฟังก์ชัน resetTimer และ startTimer ตอนกำหนดเวลาใหม่

### 1.3 ทำไมหลังจาก 4 รอบ (pomodoros) จึงมี Long Break ขนาด 15 นาที ประโยชน์คืออะไร

สมองสะสมความเหนื่อยล้าในหลายรอบสั้น
Long break ช่วยรีเฟรชสมอง ลด burnout และเพิ่มประสิทธิภาพรอบถัดไป

## ส่วนที่ 2: การจัดโครงสร้าง State

### คำถาม 2.1

ปัจจุบัน app เก็บ state อะไรบ้าง

**คำตอบ:**

- isRunning

- isWorkPhase

- timeLeft

- totalTime

- currentSession

- completedSessions

- intervalId

- workDuration / breakDuration / longBreakDuration

---

### คำถาม 2.2

ทำไมจึงต้องมี rounds state หากแค่เก็บ phase ก็พอ

**คำตอบ:**  
phase บอกแค่ว่าทำงานหรือพัก
แต่ session ใช้นับรอบเพื่อรู้ว่าเมื่อไหร่ต้องพักยาว

---

### คำถาม 2.3

ถ้าต้องการให้ Pomodoro Count อัปเดต เมื่อจบแต่ละ Work phase จะแก้ไขฟังก์ชันยังไง

**คำตอบ:**

```js
if (state.isWorkPhase) {
  state.completedSessions++;
}
```

## ส่วนที่ 3: Timer Logic

### คำถาม 3.1

อธิบายการทำงานของ setInterval() ในส่วน timer - จะเกิดอะไรขึ้นทุก 1 วินาที

**คำตอบ:**
ทุก 1 วินาที:

- ลดเวลา

- เช็คหมดหรือยัง

- -เปลี่ยน phase

- อัปเดตหน้าจอ

### คำถาม 3.2

เหตุใดจึงต้องมี timerInterval เป็น global variable ถ้าไม่มี app จะเป็นยังไง

**คำตอบ:**
เพื่อให้ pause/reset สามารถหยุด timer ได้
ถ้าไม่เก็บไว้ จะ clearInterval ไม่ได้

### คำถาม 3.3

ถ้าผู้ใช้คลิก “เริ่ม” 2 ครั้งติดต่อกัน จะเกิดอะไรขึ้น มีวิธีแก้หรือไม่

**คำตอบ:**
แก้:
if(state.isRunning) return;

## ส่วนที่ 4: DOM & UI

### คำถาม 4.1

ฟังก์ชัน updateDisplay() ทำอะไร ต้องเรียกจากที่ไหนบ้าง

**คำตอบ:**
อัปเดต:

- เวลา

- progress bar

- phase

- ปุ่ม

- stats

ต้องเรียกทุกครั้งที่ state เปลี่ยน

### คำถาม 4.2

formatTime() ใช้ padding ด้วย padStart(2, "0") เพื่ออะไร ถ้าไม่มี timer จะแสดง 5:3 แทน 05:03 ได้หรือไม่

**คำตอบ:**
เติมเลข 0 ด้านหน้าให้เป็น 2 หลัก
ไม่มี → 5:3
มี → 05:03

### คำถาม 4.3

Progress bar คำนวณเปอร์เซ็นต์ยังไง เขียนสูตรออกมา

**คำตอบ:**
progress = (เวลาที่ผ่าน / เวลาทั้งหมด) × 100

โค้ด:
(state.totalTime - state.timeLeft) / state.totalTime \* 100

## ส่วนที่ 5: Events & Controls

### คำถาม 5.1

ปุ่มควบคุมมีอะไรบ้าง แต่ละปุ่มเรียกฟังก์ชันไหน

**คำตอบ:**

- Start → startTimer

- Pause → startTimer (toggle)

- Reset → resetTimer

- Settings → change event

---

### คำถาม 5.2

ทำไม Pause ต้องหยุด timer ด้วย `clearInterval()` หากไม่ทำจะเป็นอย่างไร

**คำตอบ:**  
ไม่งั้นตัวจับเวลาจะยังทำงานอยู่แม้ UI หยุด

---

### คำถาม 5.3

Reset button ควรตั้งค่ากลับเป็นอะไร

**คำตอบ:**

- timeLeft = workDuration

- phase = work

- session = 1

- clear interval

---

## ส่วนที่ 6: Notifications & Sound

### คำถาม 6.1

ฟังก์ชัน `playSound()` ใช้เทคนิคอะไรในการสร้างเสียง

**คำตอบ:**  
ใช้ Web Audio API (Oscillator)

---

### คำถาม 6.2

ทำไม notification ควร work/break ต่างกันอย่างไร เสียง alarm ที่ดีควรเป็นยังไง

**คำตอบ:**  
ผู้ใช้แยกได้ทันทีว่า “ทำงานเสร็จ” หรือ “พักเสร็จ”
เสียงควรสั้น ชัด ไม่รบกวน

---

### คำถาม 6.3

ถ้าต้องการเพิ่ม Desktop Notification ด้วย Browser Notification API จะแก้ไขยังไง

**คำตอบ:**

```js
if (Notification.permission === "granted") {
  new Notification("Break Time!");
}
```

## ส่วนที่ 7: Progress Tracking & Stats

### คำถาม 7.1

Stats ต้องติดตามข้อมูลอะไร

**คำตอบ:**

- จำนวนรอบสำเร็จ
- เวลาทำงานรวม
- จำนวน long break

---

### คำถาม 7.2

ตำแหน่งที่ดีที่สุดในการอัปเดต stats คือที่ไหน

**คำตอบ:**  
ตอนจบ work phase

---

### คำถาม 7.3

วิธีการบันทึก stats ไว้ใน LocalStorage ได้อย่างไร

**คำตอบ:**

```js
localStorage.setItem("stats", JSON.stringify(state));
```

---

## ส่วนที่ 8: Bug Detection

### คำถาม 8.1

ถ้า timer กำลังเดินอยู่ แล้วผู้ใช้เปิด DevTools ขนาดใหญ่ timer จะยังคงเดินต่อหรือไม่

**คำตอบ:**  
timer ยังเดิน แต่ความแม่นอาจลด (tab throttling)

---

### คำถาม 8.2

ถ้า completePhase() เรียก ในขณะที่ timer ยังวิ่งอยู่ จะเกิด memory leak ได้หรือไม่

**คำตอบ:**  
เสี่ยง interval ซ้อน → memory leak

---

### คำถาม 8.3

ใน mobile ถ้าผู้ใช้ล็อก screen ขณะ timer เดิน timer จะหยุดหรือยังคงเดิน วิธีแก้คืออะไร

**คำตอบ:**  
ส่วนใหญ่ timer ช้าหรือหยุด
แก้ → คำนวณจาก timestamp แทนการนับทีละวินาที

---

## ส่วนที่ 9: Architecture & Code Quality

### คำถาม 9.1

ถ้าต้องการแยก Timer logic ออกจาก UI ควรสร้าง class อย่างไร

**คำตอบ:**  
สร้าง PomodoroTimer class จัดการเวลา
UI เรียก method เท่านั้น

---

### คำถาม 9.2

Functions ที่มีมากควรจัดไว้ใน object เพื่อจัดการได้ดีขึ้นหรือไม่

**คำตอบ:**  
ดีขึ้น เพราะจัดโครงสร้างและลด global pollution

---

### คำถาม 9.3

Code reusability - ส่วนไหนของโค้ดสามารถทำให้เป็น utility functions ได้

**คำตอบ:**

- formatTime

- playSound

- showNotification

- calculateProgress

---

## ส่วนที่ 10: Enhancements

### คำถาม 10.1

วิธีเพิ่ม Dark Mode ควรแก้ไข CSS/JS ส่วนไหนบ้าง

**คำตอบ:**

- CSS: ใช้ class dark

- JS: toggle class ที่ body

---

### คำถาม 10.2

วิธีบันทึกประวัติการใช้งาน (session history) คืออะไร

**คำตอบ:**  
บันทึก session ลง array แล้ว save LocalStorage

---

### คำถาม 10.3

วิธีเพิ่ม Keyboard Shortcuts (Space = Start/Pause, R = Reset) ควรใช้ event ไหน

**คำตอบ:**

```js
Space → start/pause
R → reset
```

# Weather App

## ส่วนที่ 1: Fetch API & HTTP Requests

### คำถาม 1.1

Fetch API เป็นวิธีมาตรฐานของ JavaScript สำหรับส่งคำขอ HTTP ไปยังเซิร์ฟเวอร์และรับข้อมูลกลับมา โดยทำงานแบบ Promise

ความแตกต่างหลัก:

| Fetch API                    | XMLHttpRequest              |
| ---------------------------- | --------------------------- |
| ใช้ Promise                  | ใช้ event/callback          |
| เขียนร่วมกับ async/await ได้ | ใช้กับ async/await ไม่สะดวก |
| โค้ดสั้น อ่านง่าย            | โค้ดยาวและซับซ้อนกว่า       |
| โครงสร้างทันสมัย             | รูปแบบเก่า                  |

### คำถาม 1.2

โค้ดนี้ทำอะไร อธิบายการทำงานแต่ละขั้นตอน

```js
fetch(url)
  .then((response) => response.json())
  .then((data) => console.log(data))
  .catch((error) => console.error(error));
```

- คำตอบ:

- fetch(url) → ส่ง HTTP request ไปยัง API

- response.json() → แปลง response เป็น JSON

- console.log(data) → แสดงข้อมูล

- .catch() → จับ error

### คำถาม 1.3

ถ้า API response ล้มเหลว (network error) จะถูก handle ที่ไหน `.catch()` จะจับได้หรือไม่

**คำตอบ:**

- หากเกิด **network error** (เช่น ไม่มีอินเทอร์เน็ต, server ปิด) → Promise จะ reject → `.catch()` จับได้
- แต่ถ้าเป็น **HTTP error** เช่น 404 หรือ 500 → `fetch()` จะยังถือว่าสำเร็จ และ `.catch()` จะไม่ทำงาน

ดังนั้นต้องตรวจเองด้วย

```js
if (!response.ok) {
  throw new Error("Request failed");
}
```

## ส่วนที่ 2: Geocoding API

### คำถาม 2.1

Geocoding API ทำหน้าที่อะไร ทำไมจึงต้องใช้ก่อนเรียก Weather API

**คำตอบ:**  
แปลงชื่อเมือง → latitude/longitude  
Weather API ต้องใช้พิกัด จึงต้องเรียกก่อน

---

### คำถาม 2.2

When user searches “Bangkok” ควรส่ง request ไปที่ endpoint ไหนก่อน ได้ข้อมูลอะไรจาก response

**คำตอบ:**  
เรียก Geocoding API  
ได้ชื่อประเทศ และพิกัด lat/lon

---

### คำถาม 2.3

ถ้า user ค้นหาเมือง “Nonthaburi” ซึ่งชื่อเดียวกับเมืองอื่น app จะตัดสินใจอย่างไร

**คำตอบ:**  
เลือก result แรกจาก API (อันดับความแม่นยำสูงสุด)

---

## ส่วนที่ 3: Weather Data & Parameters

### คำถาม 3.1

Weather API ต้องการ parameters อะไรบ้าง

**คำตอบ:**  
latitude, longitude, current, hourly, timezone

---

### คำถาม 3.2

Parameters นี้ request ข้อมูลอะไร ทำไมต้องใช้ weather_code

```js
const params =
  "?latitude=13.7&longitude=100.5&current=temperature_2m,humidity,weather_code";
```

**คำตอบ:**  
ดึงอุณหภูมิ ความชื้น และรหัสสภาพอากาศ  
weather_code ใช้แปลงเป็น icon/emoji

---

### คำถาม 3.3

ความแตกต่างระหว่าง current weather และ hourly forecast คืออะไร

**คำตอบ:**  
current = ตอนนี้  
hourly = ล่วงหน้าเป็นช่วงเวลา

---

## ส่วนที่ 4: Async/Await Pattern

### คำถาม 4.1

ทำไมจึงใช้ async/await แทน `.then().catch()`

```js
async function getWeather(city) {
  try {
    const response = await fetch(url);
    const data = await response.json();
    return data;
  } catch (error) {
    console.error(error);
  }
}
```

**คำตอบ:**  
อ่านง่าย เหมือน synchronous และจัดการ error ง่าย

---

### คำถาม 4.2

`await` ทำอะไร ถ้าไม่ใช้ code จะเป็นยังไง

**คำตอบ:**  
รอ Promise เสร็จ  
ไม่ใช้จะได้ Promise แทนข้อมูล

---

### คำถาม 4.3

`try-catch` block ใช้เพื่ออะไร

**คำตอบ:**  
จับ error จาก async operations

---

## ส่วนที่ 5: DOM Manipulation

### คำถาม 5.1

เมื่อได้ข้อมูล weather มา app ต้องแสดงผล element ใดบ้าง

**คำตอบ:**  
เมือง อุณหภูมิ ความชื้น ลม ความกดอากาศ การมองเห็น

---

### คำถาม 5.2

Loading spinner ปรากฏในไหน เมื่อไหร่ควรแสดง/ซ่อน

**คำตอบ:**  
แสดงตอน fetch  
ซ่อนเมื่อโหลดเสร็จ

---

### คำถาม 5.3

ถ้าต้องอัปเดต temperature display ทุก 10 นาที ควรใช้ setInterval() หรือ setTimeout()

**คำตอบ:**  
setInterval()

---

## ส่วนที่ 6: Data Storage (LocalStorage)

### คำถาม 6.1

LocalStorage ใช้เพื่ออะไร บันทึก recent cities ยังไง

```js
localStorage.setItem("recentCities", JSON.stringify(cities));
```

**คำตอบ:**  
เก็บข้อมูลใน browser

## ส่วนที่ 7: Error Handling & Edge Cases

### คำถาม 7.1

ถ้า user ค้นหาเมืองที่ไม่มี (เช่น “XYZ”) app จะแสดง error อย่างไร

**คำตอบ:**  
แสดง error message จาก API ว่าไม่พบเมือง

---

### คำถาม 7.2

ถ้า network ไม่มี (offline) fetch จะ throw error หรือ return success

**คำตอบ:**  
throw error แล้วไปเข้า catch

---

### คำถาม 7.3

เมื่อ hover หรือ click ที่ recent city button โปรแกรมจะทำอะไร

**คำตอบ:**  
เรียก `fetchWeather()` เพื่อโหลดข้อมูลเมืองนั้นใหม่

---

## ส่วนที่ 8: Weather Display

### คำถาม 8.1

Weather code 0 = Clear sky, 1 = Partly cloudy, 80 = Drizzle… เลือก icon/emoji ยังไง

**คำตอบ:**  
ใช้ mapping ตาราง `weather_code → emoji`

---

### คำถาม 8.2

ความแตกต่างระหว่าง อุณหภูมิ (°C), ความชื้น (%), ความเร็วลม (m/s) คืออะไร

**คำตอบ:**  
°C = ความร้อนอากาศ  
% = ปริมาณไอน้ำในอากาศ  
m/s = ความเร็วลม

---

### คำถาม 8.3

Hourly forecast แสดง 24 ชั่วโมง ต้องเลือก element ไหนจาก response

**คำตอบ:**  
`data.hourly.temperature_2m`

---

## ส่วนที่ 9: Responsive Design & Mobile

### คำถาม 9.1

ใน mobile screen weather card ควรมี layout อย่างไร

**คำตอบ:**  
จัดเรียงแนวตั้ง (vertical)

---

### คำถาม 9.2

เมื่อผู้ใช้ rotate device grid/flex ควรตัดสินใจอย่างไร

**คำตอบ:**  
ปรับ layout ตามความกว้างหน้าจอ (media query)

---

### คำถาม 9.3

Input field และ button บน mobile ต้องมี font size เท่าไร

**คำตอบ:**  
อย่างน้อย 16px เพื่อแตะง่าย

---

## ส่วนที่ 10: Code Architecture & Enhancement

### คำถาม 10.1

ถ้าต้องสร้าง Weather class เพื่อจัดการ logic อย่างไร

```js
class WeatherApp {
  async searchCity(cityName) {}
  async fetchWeather(lat, lon) {}
  displayWeather(data) {}
}
```

**คำตอบ:**
สร้าง class แยก logic API ออกจาก UI

---

### คำถาม 10.2

วิธีเพิ่ม Favorite Cities feature ยังไง ต้องแก้ไข HTML/CSS/JS ส่วนไหนบ้าง

**คำตอบ:**
เพิ่มปุ่ม favorite และเก็บใน LocalStorage

---

### คำถาม 10.3

วิธีเพิ่ม Dark Mode ด้วย CSS variables ได้อย่างไร

**คำตอบ:**
สลับ class แล้วเปลี่ยนค่า CSS variables

## 67160199 วัชระพงษ์ คำนึงการ
